...
- MyPilot 当前 daemon 主线：`开发/mypilot-link`
  - package：`@mypilot/link`
  - flavor：`mypilot-link`
  - CLI：`mypilot`
  - 本地直连端口：`52378`
- ClawPilot 公共发布包线：`开发/package`
...
- MyPilot App 私有化体验、诊断、定时任务、附件协议、本地 OpenClaw 调试：优先修改 `开发/mypilot-link`。
- ClawPilot 公共 npm 发布包、`clawlink` CLI、公共发布流程：修改 `开发/package`。
- 不要把 MyPilot 专属能力无差别复制到 `开发/package`。
...
```bash
cd /Users/liaoxing/Downloads/未命名文件夹/开发/mypilot-link
npm run verify
```

```bash
cd /Users/liaoxing/Downloads/未命名文件夹/开发/package
npm run verify
```
